...
function renderSessionSummary(
  ...
  return html`
    ...
    <div class="session-summary-grid">
      ...
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.total").replace(":", "")}</div>
    ...
    <div class="usage-insights-grid" style="margin-top: 12px;">
      ${renderInsightList("Top Tools", toolItems, t("usage.noToolCalls"))}
      ${renderInsightList("Model Mix", modelItems, t("usage.noModelData"))}
    </div>
  `;
}
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.loadingTimeline")}</div>
      </div>
    `;
  }
        ...
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px; color: var(--text);">${t("usage.tokensByType").replace("{mode}", t("usage.tokens"))}</div>
  ...
  return html`
    <div class="context-details-panel">
      ...
      <div class="context-legend">
        ...
        <span class="legend-item"><span class="legend-dot files"></span>${t("usage.files")} ~${formatTokens(filesTokens)}</span>
      </div>
...
function renderSessionLogsCompact(
  ...
  const displayedCount =
    hasActiveFilters || hasCursorFilter
      ? `${filteredEntries.length} of ${logs.length} ${hasCursorFilter ? `(${t("usage.timelineFiltered")})` : ""}`
      : `${logs.length}`;
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.conversation")} <span style="font-weight: normal; color: var(--muted);">(${displayedCount} messages)</span></span>
